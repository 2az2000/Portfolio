...
- **Goal of the page:** Instead of _telling_ the visitor "I'm a good frontend developer," the site should _show_ it. The experience of using the site — the micro-interactions, performance, animation quality, attention to detail — is itself the proof of skill.
...
Instead of a classic hero (big headline + photo + button), the hero contains several **real, clickable mini components** floating in an irregular bento layout (a toggle switch, a badge, a small slider, a progress ring) that the visitor can actually play with. Skill is _experienced_, not described.
...
Each card has a small mono-font label above it styled like a filename (`spotlight-card.tsx`, `magnetic-button.tsx`) — this is a meaningful structural device: this section literally _is_ live code, not decoration.
...
> **Important constraint:** per the "spend your boldness in one place" principle, these ideas don't compete with each other. Color and motion language stay calm and consistent across every section; only the _structure_ of each section is distinct and meaningful.
...
/app
  (en)/                → English root layout + `/`
    layout.tsx         → <html lang="en" dir="ltr"> + English metadata + JSON-LD
    page.tsx           → renders <PageContent />
    opengraph-image.jpg
  (fa)/                → Persian root layout + `/fa`
    layout.tsx         → <html lang="fa" dir="rtl"> + Persian metadata
    fa/page.tsx        → renders <PageContent />
    fa/opengraph-image.jpg
  not-found.tsx        → 404. Sits above both groups, so it brings its own styling
  sitemap.ts robots.ts manifest.ts
  api/contact/route.ts → the only server route (Resend)
  fonts/               → subset woff2 consumed by next/font/local
  globals.css          → @tailwind directives + global utilities (glass, aurora-text, ...)
  icon.png apple-icon.png

/components
  RootShell.tsx        → the shared <html>/<body>, font loaders, chrome. Both root
                         layouts render this; it is where the duplication stops.
  PageContent.tsx      → section composition, rendered by both locale routes
  StructuredData.tsx   → Person + WebSite JSON-LD, derived from the dictionaries
  /ui                  → reusable atoms (no business logic)
    SpotlightCard.tsx MagneticButton.tsx CommandPalette.tsx
    SkillsGraph.tsx SkillNode.tsx SkillsMobile.tsx
    DocumentPreview.tsx TechMarquee.tsx SplineScene.tsx
  ...
  /sections            → full page sections (each wraps a <section id="...">)
    ...
    CaseStudies.tsx Experience.tsx Contact.tsx
  Navbar.tsx Footer.tsx CustomCursor.tsx AmbientBackground.tsx
  Providers.tsx SmoothScroll.tsx ThemeWipe.tsx
  LanguageProvider.tsx → Context for fa/en + dir + dictionary access
  LocaleCurtain.tsx    → the full-screen wipe covering the locale swap
  XRayProvider.tsx XRayRegion.tsx XRayLegend.tsx

/lib
  site.ts              → SITE_URL, per-locale metadata, hreflang alternates
  theme.ts             → motion/easing constants for GSAP (mirrors Tailwind tokens)
  useGsapReveal.ts smoothScroll.ts themeWipe.ts scrollVelocity.ts
  navSections.ts fuzzy.ts xray.ts utils.ts
  i18n/
    en.ts              → declares the Dictionary *type* and the English copy
    fa.ts              → Persian dictionary (must satisfy that type)
    index.ts

/public                → served verbatim. Keep it to what the site actually requests.
  fonts/ images/ resume/

/assets                → build inputs, never served (source TTFs, full-size images)
```

**Routing note.** Two root layouts exist because only a root layout may render
`<html>`, and `lang`/`dir` must be correct in the server response for each
locale. The in-page language toggle still never navigates between them — it
swaps the dictionary behind the curtain and calls `history.replaceState`,
because routing would remount the tree and tear down every GSAP ScrollTrigger
and the Spline WebGL context. See the comment on `setLocale` in
...
- **button / badge** → use for anything that is _not_ the hero CTA or a playground demo (those use our own `MagneticButton` / `BadgeProof` on purpose, since they're signature moments).
